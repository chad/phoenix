...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Platform listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/platform/server.js') ||
               process.argv[1]?.endsWith('/platform/server.ts');
if (isMain) {
  startServer();
}
