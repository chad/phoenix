...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Operations listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/operations/server.js') ||
               process.argv[1]?.endsWith('/operations/server.ts');
if (isMain) {
  startServer();
}
