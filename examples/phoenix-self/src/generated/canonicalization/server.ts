...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Canonicalization listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/canonicalization/server.js') ||
               process.argv[1]?.endsWith('/canonicalization/server.ts');
if (isMain) {
  startServer();
}
