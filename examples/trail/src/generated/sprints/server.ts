...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  const requestedPort = port ?? parseInt(process.env.SPRINTS_PORT ?? process.env.PORT ?? '3002', 10);
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Sprints listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/sprints/server.js') ||
               process.argv[1]?.endsWith('/sprints/server.ts');
if (isMain) {
  startServer();
}
