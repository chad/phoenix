...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  const requestedPort = port ?? parseInt(process.env.BOARD_PORT ?? process.env.PORT ?? '3000', 10);
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Board listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/board/server.js') ||
               process.argv[1]?.endsWith('/board/server.ts');
if (isMain) {
  startServer();
}
