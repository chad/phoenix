...
const _svcModules = {
  'issues': issues,
  'validation-and-workflow-rules': validationAndWorkflowRules,
};
...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  const requestedPort = port ?? parseInt(process.env.ISSUES_PORT ?? process.env.PORT ?? '3001', 10);
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Issues listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/issues/server.js') ||
               process.argv[1]?.endsWith('/issues/server.ts');
if (isMain) {
  startServer();
}
