...
const _svcModules = {
  ...
  'creating-an-expense': creatingAnExpense,
  'expense-history': expenseHistory,
  'split-strategies': splitStrategies,
};
...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  const requestedPort = port ?? parseInt(process.env.EXPENSES_PORT ?? process.env.PORT ?? '3001', 10);
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Expenses listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/expenses/server.js') ||
               process.argv[1]?.endsWith('/expenses/server.ts');
if (isMain) {
  startServer();
}
