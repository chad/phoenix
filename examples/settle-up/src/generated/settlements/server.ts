...
const _svcModules = {
  'debt-simplification': debtSimplification,
  'recording-settlements': recordingSettlements,
...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  const requestedPort = port ?? parseInt(process.env.SETTLEMENTS_PORT ?? process.env.PORT ?? '3003', 10);
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Settlements listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/settlements/server.js') ||
               process.argv[1]?.endsWith('/settlements/server.ts');
if (isMain) {
  startServer();
}
