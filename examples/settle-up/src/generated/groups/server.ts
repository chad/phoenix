...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  const requestedPort = port ?? parseInt(process.env.GROUPS_PORT ?? process.env.PORT ?? '3002', 10);
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Groups listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/groups/server.js') ||
               process.argv[1]?.endsWith('/groups/server.ts');
if (isMain) {
  startServer();
}
