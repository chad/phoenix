...
describe('capability eval — the system\'s Red/Green trust surface', () => {
  it('has no regressions: every green-expected capability still passes', async () => {
    const sc = await runSuite(CAPABILITY_SUITE);
    const broken = sc.regressions.map(r => `${r.case.id}: ${r.detail}`);
    expect(broken, `REGRESSIONS (proven capabilities that broke):\n${broken.join('\n')}`).toEqual([]);
  });

  it('green-health is 100% (kept promises are all kept)', async () => {
    const sc = await runSuite(CAPABILITY_SUITE);
    expect(sc.greenHealth).toBe(1);
  });

  it('surfaces promotions loudly (reds that now pass — flip them to green)', async () => {
    const sc = await runSuite(CAPABILITY_SUITE);
  ...
  it('every red documents its reason (the backlog explains itself)', async () => {
  ...
  it('scorecard totals are internally consistent', async () => {
    const sc = await runSuite(CAPABILITY_SUITE);
